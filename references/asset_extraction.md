...
本文件是 crop 语义、异常触发和裁剪修复的唯一权威。总体路线先由 `routing.md` 的首图勘察锁定；不要先裁出全部小图再反推资产路线。
...
助手脚本不做完美对象分割，只裁 manifest 提供的矩形 `source_region`。`edge_check` 报告四边是否仍有强视觉信号；它不证明语义精确，也不能可靠检测窗口内的无关邻居。首图勘察负责初始语义，contact sheet 负责批量覆盖，`crop_window_consistency` 负责事后像素兜底。
