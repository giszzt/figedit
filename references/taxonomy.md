...
本文件只定义分类词汇和模式映射。总体路线、编辑深度和验证档位由 `routing.md` 的勘察决定；不要按本文件的类别逐轮重看源图。
...
模式 E 是组合模式，不再要求整图只能标成 `E-ai`。源图局部显示强 AI 信号时，在勘察中直接建立区域 scope，不要等 SVG 或裁剪失败后再选。

## 模式选择规则

在一次勘察中批量完成模式映射，不要先逐元素处理再回来选模式：
