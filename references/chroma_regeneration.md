...
- **与色相分区不冲突**：只有 Route Decision 已 `ready`、用户检查点已解决后，`probe_palette.py --boxes` 才按客观撞色判据在生成前划分 sheet，属于执行规划且必要（见下文 Pipeline 第 1 步）。检查点前的预算只按整图保守估计；本节禁止的是按数量或尺寸臆测拆分。
