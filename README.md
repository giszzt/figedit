...
**图片原始可编辑版本丢失？** — 设计师交付的精美信息图，可编辑源文件丢了或从未共享。FigEdit 拆成可编辑的 SVG，框架变矢量，图标按可分离性通过干净裁剪、污染再生或区域压平保真，文字变成可选中的文本。
...
- Logo、照片、截图、地图、复杂图标等来源特异的视觉内容先保留身份：窗口干净才裁剪，污染时再生、压平或保留完整区域；
...
| 图标、照片、地图、图表、Logo | 按可分离性裁剪、再生或区域保留，保持视觉身份 |
| 被遮挡或纠缠的图形对象      | AI 在色键底上再生，抠出为干净透明资产         |
| 复杂连续背景           | 生成擦除前景、补全像素的 AI 清版底板         |

模型先整图判断，再按区域和对象组合重建策略：确定性结构用 SVG，局部或全图连续场用区域清版底，干净的专有素材裁剪，受压盖污染的素材再生或压平。走清版区域时，模型会先给出前景处理方案（全部提取为独立资产、只提取指定对象、或保留在底板中压平）供确认，再发起生成。
...
| Agent 环境        | 推荐模型                               | 说明                                                                               |
| --------------- | ---------------------------------- | -------------------------------------------------------------------------------- |
| **Codex**       | GPT 6 Astra                            | 优先推荐，视觉理解、空间推理与工具调用能力稳定，速度快，且自带 image_gen 图像生成，清版路线开箱即用                              |
| **Claude Code** | Claude Fable 5、Opus5.5                    | SVG 绘制能力最强，复杂图形的路径组织、文本布局和层级关系处理更稳，适合追求高质量可编辑 SVG 的重建任务                         |
| **其他主流 Agent**  | GPT-5.6、Claude Fable 5、Gemini 3.5 | 不建议使用仅擅长代码、但缺少图片输入或空间视觉推理能力的模型。即使能够执行 FigEdit 脚本，也容易在元素分类、裁切边界、层级关系和布局判断上出现明显偏差。 |
...
**指定区域或对象路线**，SVG、区域清版、裁剪与再生可以按需组合：
