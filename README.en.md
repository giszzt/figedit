...
- photos, maps, screenshots, logos, and distinctive visual elements keep their visual identity through clean crops, regeneration, flattening, or preserved self-contained regions;
...
If the editable source of a designed infographic has been lost or was never shared, FigEdit can recover much of its structure from the flattened image. Frames become vectors, readable labels become selectable text, and source-specific icons or imagery are preserved through clean crops, regeneration, or regional raster handling according to separability.
...
- logos, photos, screenshots, maps, complex icons, and other source-specific visuals are cropped only when cleanly separable; contaminated objects are regenerated or flattened, and self-contained evidence regions can remain rasterized;
...
The agent reads the whole figure first, then composes a route by region and object: deterministic structure becomes SVG, local or full-canvas continuous fields use regional clean plates, clean source-specific assets are cropped, and visibly contaminated assets are regenerated or flattened. For each clean-plate region, the agent first presents the foreground options — extract every pictorial object as an independent asset, extract a named subset, or keep them flattened in the plate — before any generation call.
...
**Choosing regional or object routes** — SVG, regional clean plates, crops, and regeneration can be combined:
